    ...
    int calPoints(vector<string>& operations) {
        ...
        for (string op : operations) { // Part01: C++ 進階迴圈
            ...
            if (op[0]=='C') { // Clear 清掉最後一位
                a.pop_back(); //丟掉最後一個 // Part05: .pop_back()
            } else if (op[0]=='D') { // 最後1位變2倍再加到最後面
                ...
                a.push_back( temp + temp2 ); // 把最後2個加起來, 再加到最後面
            } else { // 數字的字串囉, 要 加到最後面
                a.push_back( stoi(op) ); // Part04: .push_back()
            }
        }
        int ans = 0;
        for(int now : a) { // Part05: C++ 進階迴圈 要看陣列的值, 把全部加起來
            ans += now; //cout << now << " "; // Part02: 看他是誰, 等一下刪掉哦(看陣列的值)
        }

        return ans; //return 0; // 先隨便 return 等一下再寫真的答案
    }
};
